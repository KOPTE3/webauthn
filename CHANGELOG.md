...
* В методах `open` и `wait` исправлено обращение к `this.page.locators.container`, которое могло вызывать экспепшены и тратить процессорное время на выполнение асхинхронно кода, который может быть размещен в поле `locators`.
