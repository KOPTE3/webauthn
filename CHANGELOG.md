...
Но такой способ запуска не следует использовать в CI (лучше вообще не использовать), поскольку будет использоваться не локальная версия grunt'а, а глобальная.
